...
function cleanup () {
  ...
  removeSync(process.env.STUFF_FOLDER_SRC)
  removeSync(process.env.STUFF_FOLDER_THEME)
 ...
 *  - process.env.STUFF_FOLDER_SRC : The path to the safe workable source folder
 *  - process.env.STUFF_FOLDER_THEME : The path to the safe workable theme folder
 *  - process.env.STUFF_FOLDER_REVEAL : The path to the NPM reveal.js folder
 *
...
async function setup (src, theme) {
  ...
  process.env.STUFF_FOLDER_REVEAL = require
    .resolve(`reveal.js`)
    .replace(/(node_modules\/reveal\.js).*/, `$1`)

  // Let's create a safe theme folder
  process.env.STUFF_FOLDER_THEME = await createSafeThemeFolder(src, theme)

  // Let's try to compile our Sass file into our safe folder
  // That way we do not cluter the original theme folder
  const files = (await readdir(process.env.STUFF_FOLDER_THEME))
    .filter(is.file.sass)
    .map(file => path.join(process.env.STUFF_FOLDER_THEME, file))
  ...
  process.env.STUFF_FOLDER_SRC = await createSafeSourceFolder(
    src,
    process.env.STUFF_FOLDER_THEME
  )
