...
async function cmd (dest) {
  const SRC_DIR = process.env.STUFF_FOLDER_SRC
  const THEME_DIR = process.env.STUFF_FOLDER_THEME
  ...
  await ensureLink(dest, path.join(process.env.STUFF_FOLDER_SRC, path.basename(dest)))
}
