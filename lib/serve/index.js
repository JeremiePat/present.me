...
async function setup (src, log, options) {
  ...
  if (options.theme && fs.existsSync(path.resolve(options.theme))) {
    ...
    // We try to unlink the theme folder if it exists in order
    // to replace it with the one define through the options
    await unlink(path.join(tmpDir, `theme`)).catch(() => {
      /* If there is nothing to unlink, we just want to swallow this error */
      log.debug.info(_`No previous theme to unlink, let's move on`)
    })

    // And we link the new one
    await fs.ensureSymlink(path.resolve(options.theme), path.join(tmpDir, `theme`))
  }
