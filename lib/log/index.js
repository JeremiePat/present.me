...
class Logger extends Writable {
  constructor (opt) {
    ...
    // Bind public interface so that it can be "detached"
    ...
    // Set up a default log method when used as a stream
    if (!this.streamLogger) {
      this.streamLogger = this.info
    }

    // Setup special debug formater
  ...
  _log (prefix, ...args) {
    ...
    // Get optimal lineLength between 1 and Logger.MAX_TTY_LENGTH
    const lineLength = minmax(
  ...
  /**
   * Special log method that must log no matter what
   *
  ...
  /**
   * add a new line (\n) in the log stream if not silent
   */
  nl () {
    if (this.verbose !== Logger.VERBOSITY_SILENT) {
      this.tty.write(`\n`)
  ...
  /**
   * Clear the current line in overwrite mode
   */
  clear () {
